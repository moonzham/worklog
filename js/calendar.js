...
function buildReportPrompt(payload){
  ...
  const commonRules=[
    ...
    '- 데이터에 없는 내용은 추측하지 말고, 확인되지 않은 완료/배포/일정은 단정하지 않는다. 진행 사항이 없으면 무조건 생략한다.',
    ...
    '- 제목 줄 없이 본문만 작성한다.',
    '- 하위 상세 리스트('-' 기호 뒷부분) 내에는 issueNo(예: OPD-222, DAR-111 등)나 issueTitle을 중복하여 포함하지 않는다. 대시(-) 뒤에는 오직 순수한 작업 내용 및 서술형 문장만 작성한다.',
    '- 주차별 이슈 없는 일반 작업은 "기타 작업" 항목 하나로만 묶는다.',
    '- 해당 주차의 journals[].content 원문에 직접적인 작업 언급(이슈코드 혹은 관련 키워드)이 전혀 없고, 동시에 이번 주차에 시작된 이슈(reportStartDate가 이번 주차 기간에 포함되는 이슈)가 아니라면, 진행 사항이 없는 것으로 간주하여 보고서에서 완전히 제외(생략)한다.',
  ...
  if(payload.reportType==='week'){
    formatRules=[
      ...
      ' - 수행한 작업 내용과 확인된 결과가 있으면 1~2줄로 요약한다. (※ 문장 내에 이슈코드, 이슈명을 다시 쓰지 말고, 현재 진행 중인 작업 내용을 자연스러운 문장으로 요약할 것)',
      '',
      '<진행 중 업무>',
      '* [이슈코드] 이슈명 (이슈코드가 빈값인 경우 [이슈명])',
      ' - 현재 진행 중인 작업과 다음에 이어서 할 작업을 한 문장으로 요약한다. (※ 문장 내에 이슈코드, 이슈명을 다시 쓰지 말고, 현재 진행 중인 작업 내용을 자연스러운 문장으로 요약할 것)',
      '',
      '<리스크/특이사항>',
    ...
    formatRules=[
      ...
      '<1주차>',
      '* [이슈코드] 이슈명 (이슈코드가 빈값인 경우 [이슈명])',
      ' - 작업한 내용과 진행 상황, 성과, 이슈, 지연사항이 있으면 1~2줄로 요약한다. (※ 문장 내에 이슈코드, 이슈명을 다시 쓰지 말고, 현재 진행 중인 작업 내용을 자연스러운 문장으로 요약할 것)',
      '',
      '<2주차>',
      '* [이슈코드] 이슈명 (이슈코드가 빈값인 경우 [이슈명])',
      ' - 작업한 내용과 진행 상황, 성과, 이슈, 지연사항이 있으면 1~2줄로 요약한다. (※ 문장 내에 이슈코드, 이슈명을 다시 쓰지 말고, 현재 진행 중인 작업 내용을 자연스러운 문장으로 요약할 것)',
